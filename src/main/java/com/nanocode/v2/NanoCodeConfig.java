...
import com.nanocode.tools.annotation.ToolScanner;
 ...
 *     |
 *     +-- 核心引擎（独立配置类）:
 *           +-- AgentLoopConfig -> 8 状态 State 模式引擎 (see {@link com.nanocode.v2.AgentLoopConfig})
 * </pre>
 *
 * <h2>每次对话的运行时流程</h2>
 * <pre>
...
public class NanoCodeConfig {
    ...
    public PermissionManager permissionManager(PathGuard pathGuard,
        ...
        com.nanocode.security.rule.RuleEngine ruleEngine = new com.nanocode.security.rule.RuleEngine();
        // Shell 命令需要用户确认
        ruleEngine.addRule(com.nanocode.security.rule.RuleType.ASK, "exec", null, null,
                "Shell 命令执行需要您的确认");
        // rm -rf 需特别确认
        ruleEngine.addRule(com.nanocode.security.rule.RuleType.ASK, "exec", "command", "rm -rf.*",
                "递归删除操作需要确认");
     ...
     * </pre>
     *
     * @see com.nanocode.hook.HookManager
     * @see com.nanocode.hook.Hook
     */
    @Bean
