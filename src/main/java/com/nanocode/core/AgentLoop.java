package com.nanocode.core;

import com.nanocode.bus.*;
...
public class AgentLoop implements AutoCloseable {
     ...
     * State 模式处理器注册表
     */
    private final java.util.Map<TurnState, com.nanocode.core.state.AgentState> stateHandlers =
     ...
     */
    private com.nanocode.memory.Consolidator consolidator;
    ...
    public com.nanocode.memory.Consolidator getConsolidator() { return consolidator; }
    public com.nanocode.memory.Dream getDream() { return dream; }
...
//上次心跳时间
    ...
    private TurnState processState(TurnContext context, TurnState state) {
        ...
        com.nanocode.core.state.AgentState handler = stateHandlers.get(state);
        if (handler == null) {
    ...
    private void initStateHandlers() {
        stateHandlers.put(TurnState.RESTORE, new com.nanocode.core.state.RestoreState(sessionManager));
        stateHandlers.put(TurnState.COMPACT, new com.nanocode.core.state.CompactState(consolidator));
        stateHandlers.put(TurnState.COMMAND, new com.nanocode.core.state.CommandState(this, skillManager));
        stateHandlers.put(TurnState.BUILD, new com.nanocode.core.state.BuildState(identityManager, ruleManager,
                () -> planMode, dream, skillManager != null ? skillManager.getRegistry() : null,
                config.getWorkspacePath(), hookManager));
        stateHandlers.put(TurnState.RUN, new com.nanocode.core.state.RunState(runner, config, messageBus, hookManager));
        stateHandlers.put(TurnState.SAVE, new com.nanocode.core.state.SaveState(sessionManager));
        stateHandlers.put(TurnState.RESPOND, new com.nanocode.core.state.RespondState(messageBus));
    }

    /**
     * 设置记忆压缩器，同时更新 CompactState 和 CommandState 处理器
     */
    public void setConsolidator(com.nanocode.memory.Consolidator c) {
        this.consolidator = c;
        stateHandlers.put(TurnState.COMPACT, new com.nanocode.core.state.CompactState(c));
        stateHandlers.put(TurnState.COMMAND, new com.nanocode.core.state.CommandState(this, skillManager));
    }
     ...
     */
    public void setDream(com.nanocode.memory.Dream d) {
        this.dream = d;
        stateHandlers.put(TurnState.SAVE, new com.nanocode.core.state.SaveState(sessionManager, d));
        stateHandlers.put(TurnState.BUILD, new com.nanocode.core.state.BuildState(identityManager, ruleManager,
