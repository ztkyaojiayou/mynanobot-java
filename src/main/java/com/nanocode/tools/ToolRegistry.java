package com.nanocode.tools;

import com.fasterxml.jackson.databind.JsonNode;
import com.nanocode.security.PermissionManager;
import com.nanocode.security.PermissionResult;
import com.nanocode.security.guard.CommandGuard;
import com.nanocode.security.guard.NetworkGuard;
import com.nanocode.security.guard.PathGuard;
import com.nanocode.security.guard.SecurityException;
...
public class ToolRegistry {
    ...
    /**
     * 执行工具 — 查找 → 验证 → 权限检查 → 执行 → 包装结果.
     *
     * <h3>执行流程（4 步）</h3>
     * <ol>
     *   <li>{@link #findToolOrThrow} — 查找工具</li>
     *   <li>{@link #validateParams} — 验证参数</li>
     *   <li>权限检查（PermissionManager 或旧版 applyGuards）</li>
     *   <li>{@link #executeAndWrapResult} — 执行工具 + 异常包装</li>
     * </ol>
     *
     * @param name   工具名称
     * @param params 工具参数
    ...
    public Object execute(String name, Map<String, Object> params) {
        ...
        try {
            // ① 查找工具（不存在时抛 ToolNotFoundException）
            Tool tool = findToolOrThrow(name);

            // ② 验证参数
            String validationError = validateParams(tool, name, params);
            if (validationError != null) return ToolResult.err(validationError);

            // ③ 权限检查
            if (permissionManager != null) {
                PermissionResult permResult = permissionManager.check(tool, params);
                if (permResult.isDenied()) {
                    return ToolResult.err("Permission denied: " + permResult.getReason());
                }
            } else {
            ...
            return executeAndWrapResult(tool, name, params);
    ...
    /** ① 查找工具，不存在时抛 {@link ToolNotFoundException} */
    private Tool findToolOrThrow(String name) {
        Tool tool = tools.get(name);
        if (tool == null) {
            String error = String.format("Tool '%s' not found. Available tools: %s",
                    name, String.join(", ", tools.keySet()));
            logger.warn(error);
            throw new ToolNotFoundException(error);
        }
        return tool;
    }

    /** ② 验证参数，返回 null 表示通过，否则返回错误消息 */
    private String validateParams(Tool tool, String name, Map<String, Object> params) {
        List<String> errors = tool.validateParameters(params);
        if (errors.isEmpty()) return null;

        String hint = params.isEmpty()
                ? " (参数为空，请检查是否忘记传参，如 list_dir 可不传参使用默认值)"
                : "";
    ...
    /** ④ 执行工具并包装结果（含 CompletionException 和通用异常处理） */
    private Object executeAndWrapResult(Tool tool, String name, Map<String, Object> params) {
        try {
