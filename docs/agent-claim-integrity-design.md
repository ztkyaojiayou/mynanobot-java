...
> 背景：用户实测发现 NanoCode 出现"声称推送成功但实际未执行"的真实性缺陷。
...
| 维度 | Claude Code | NanoCode（本期设计） |
|------|------------|---------------------|
| 工具执行真实性 | 管道强制（tool_use 发射即执行） | 已有（同基线） |
| 防"文本声称完成" | 无工程检测，依赖模型自觉 | **三信号硬检测，拦截重试** |
| 事后验证 | 门控在内部员工，社区不可用 | 可选增强（见六） |

**核心差异化**：把社区只做成提示词（Iron Law）的规则，落地为**框架硬检测**——不依赖模型自觉，这是 NanoCode 相对 Claude Code 的真实机会。
